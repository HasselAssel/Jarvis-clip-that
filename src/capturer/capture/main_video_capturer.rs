use crate::capturer::capture::recorder::{BaseParams, Recorder};
...
impl<P: PacketRingBuffer> VideoCapturer<P> {
    pub fn new(ring_buffer: Arc<Mutex<P>>, monitor: u32) -> Result<(Self, Parameters), IdkCustomErrorIGuess> {
        ...
        unsafe {
            ...
            output = adapter.EnumOutputs(monitor).unwrap();
        }
        ...
        let par = Parameters::from(&video_encoder);
...
impl<P: PacketRingBuffer> Recorder<P> for VideoCapturer<P> {
    fn start_capturing(mut self) -> JoinHandle<Result<(), IdkCustomErrorIGuess>> {
        thread::spawn(move || -> Result<(), IdkCustomErrorIGuess> {
            ...
            let mut frame: ffmpeg_next::frame::Video = ffmpeg_next::frame::Video::new(ffmpeg_next::format::Pixel::NV12, self.out_width, self.out_height);
            ...
            loop {
                ...
                for i in 0..u32::MAX {
                    ...
                    Self::send_frame_and_receive_packets(&self.ring_buffer, &mut self.video_encoder, &frame, 1);
                }
            }
        })
    }

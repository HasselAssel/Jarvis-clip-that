...
impl PacketHandler for KeyFrameStartPacketWrapper {
    fn insert(container: &mut VecDeque<Self>, packet: Packet) {
        let needs_new = container.back().map_or(true, |item| {
            item.buffer
                .last()
                .expect("Corrupted KeyFrameStartPacketWrapper (no inner element)")
                .is_key()
        });
        ...
        let target_self = container
            .back_mut()
            .expect("Corrupted KeyFrameStartPacketWrapper Container (doesn't contain element)");

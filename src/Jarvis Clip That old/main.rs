...
use crate::recorders::recorder::create_video_recorder;
...
async fn main_async() {
    ...
    let mut video_recorder = create_video_recorder::<VideoPacketRingBufferType>(
        &video_source_type,
        &video_codec,
        seconds,
        2560,
    ...
    .unwrap();
    let mut audio_recorder_input = create_audio_recorder::<AudioPacketRingBufferType>(
        &audio_source_type,
        &audio_codec,
        seconds,
        0.,
    ...
    let mut audio_recorder =
        AudioProcessWatcher::<AudioPacketRingBufferType>::new(audio_codec, true, seconds, 0.)
            .unwrap();
    ...
    audio_recorder
        .start_recording()
        .await
        .unwrap_or_else(|err| panic!("Failed start_recording because: {:?}", err));

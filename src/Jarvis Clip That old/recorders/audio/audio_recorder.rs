...
impl<PRB: PacketRingBuffer + 'static, AS: AudioSource + Send + 'static> TRecorder<PRB>
    for AudioRecorder<PRB, AS>
{
    fn start_capturing(
        mut self: Box<Self>,
        stop_capturing_callback: Option<Arc<AtomicBool>>,
    ) -> RecorderJoinHandle {
        fn help<PRB: PacketRingBuffer, AS: AudioSource + Send>(
            selbst: &mut Box<AudioRecorder<PRB, AS>>,
        ) {
            ...
            selbst
                ...
                .gather_new_audio(
                    &selbst.ring_buffer,
                    &mut selbst.audio_encoder,
                    &mut selbst.frame,
                    &mut selbst.silent_frame,
                )
                .unwrap_or_else(|err| {
                    panic!(
                        "AudioRecorder: Failed to send_frame_and_receive_packets because: {:?}",
                        err
                    )
                });
        }

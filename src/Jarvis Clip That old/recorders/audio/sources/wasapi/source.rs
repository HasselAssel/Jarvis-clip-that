...
use crate::recorders::audio::sources::enums::AudioCodec;
use crate::recorders::audio::sources::enums::AudioSourceType;
use crate::recorders::audio::sources::traits::AudioSource;
use crate::recorders::audio::sources::wasapi::traits::WasapiEncoderCtx;
use crate::recorders::recorder::create_audio_recorder;
use crate::recorders::recorder::Recorder;
use crate::ring_buffer::traits::PacketRingBuffer;
use crate::types::Result;
use crate::wrappers::MaybeSafeComWrapper;
use crate::wrappers::MaybeSafeHANDLEWrapper;
...
impl<E: WasapiEncoderCtx> AudioSource for AudioSourceWasapi<E> {
    ...
    fn gather_new_audio<PRB: PacketRingBuffer>(
        ...
        self.context_encoder.process_audio(
            ring_buffer,
            encoder,
            frame,
            silent_frame,
...
pub fn create_default_iaudioclient(
...
pub struct AudioProcessWatcher<PRB: PacketRingBuffer> {
    pub audio_recorders:
        Arc<tokio::sync::Mutex<HashMap<u32, (Recorder<PRB>, String, Arc<AtomicBool>)>>>,
...
impl<PRB: PacketRingBuffer + 'static> AudioProcessWatcher<PRB> {
    pub fn new(
        ...
        Ok(Self {
            audio_recorders,
            _audio_process_watcher: Some(_AudioProcessWatcher::new(
                audio_codec,
                include_tree,
                min_secs,
                a,
...
impl<PRB: PacketRingBuffer + 'static> _AudioProcessWatcher<PRB> {
    fn new(
        ...
        min_secs: u32,
        audio_recorders: Arc<
            tokio::sync::Mutex<HashMap<u32, (Recorder<PRB>, String, Arc<AtomicBool>)>>,
        >,
        start_delay_secs: f64,
        start_instant: Instant,
    ) -> Result<Self> {
    ...
    async unsafe fn try_add_new_process(&mut self, p_id: u32, start_delay_secs: f64) -> Option<()> {
        ...
        /*debug_println!*/
        eprintln!("Added: PID: {p_id}, {p_name}");
    ...
    pub async fn start_listening(mut self) -> Result<()> {
        ...
        let session_handle: WinAudio::IAudioSessionNotification = SessionNotification {
            add_process_tx,
            _remove_process_tx,
            session_events,
        }
        ...
        let _ = unsafe {
            &self
                .session_manager
                .RegisterSessionNotification(&session_handle)
        };
        ...
        tokio::spawn(async move {
            while let Some(p_id) = add_process_rx.recv().await {
                ...
                if let Some(_) =
                    unsafe { self.try_add_new_process(p_id, delay + self.start_delay_secs) }.await
                {
                    let mut audio_recorders = self.audio_recorders.lock().await;
    ...
    fn get_process_name(pid: u32) -> Option<String> {
        unsafe {
            ...
            .ok()?;
            let mut entry =
                windows::Win32::System::Diagnostics::ToolHelp::PROCESSENTRY32::default();
            entry.dwSize =
                size_of::<windows::Win32::System::Diagnostics::ToolHelp::PROCESSENTRY32>() as u32;

            if let Ok(_) =
                windows::Win32::System::Diagnostics::ToolHelp::Process32First(snapshot, &mut entry)
            {
                loop {
                    ...
                    if let Err(_) = windows::Win32::System::Diagnostics::ToolHelp::Process32Next(
                        snapshot, &mut entry,
                    ) {
                        break;
                    }
    ...
    session_events: Arc<
        Mutex<
            HashMap<
                u32,
                (
                    MaybeSafeComWrapper<WinAudio::IAudioSessionEvents>,
                    MaybeSafeComWrapper<IAudioSessionControl2>,
                ),
            >,
        >,
    >,
...
impl WinAudio::IAudioSessionNotification_Impl for SessionNotification_Impl {
    fn OnSessionCreated(
        &self,
        newsession: windows::core::Ref<'_, WinAudio::IAudioSessionControl>,
    ) -> windows_core::Result<()> {
        if let Some(new_session) = newsession.as_ref() {
            ...
            self.session_events.lock().unwrap().insert(
                p_id,
                (
                    MaybeSafeComWrapper(session_events),
                    MaybeSafeComWrapper(new_session2),
                ),
            );

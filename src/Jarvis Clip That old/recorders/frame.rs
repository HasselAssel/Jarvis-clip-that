...
use ffmpeg_next::sys::av_frame_alloc;
use ffmpeg_next::sys::av_hwframe_get_buffer;
...
pub fn create_audio_frames(format: Sample, size: usize, layout: ChannelLayout) -> (Audio, Audio) {
    let frame = Audio::new(format, size, layout);
    let mut silent_frame = Audio::new(format, size, layout);
    let buf = vec![0u8; size * silent_frame.format().bytes() * silent_frame.channels() as usize];
    debug_println!(
        "SIZE: {}",
        size * silent_frame.format().bytes() * silent_frame.channels() as usize
    );
    unsafe {
        copy_into_audio_frame(&mut silent_frame, buf);
    }

    (frame, silent_frame)
}

pub unsafe fn copy_into_audio_frame(frame: &mut Audio, buffer: Vec<u8>) {
    ...
    let planes: Vec<*mut u8> = (0..num_channels)
        .map(|i| (*frame.as_ptr()).extended_data.add(i).read())
        .collect();

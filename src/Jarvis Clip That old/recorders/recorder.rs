...
use ffmpeg_next::sys::AVPixelFormat::AV_PIX_FMT_QSV;
...
use crate::recorders::audio::sources::wasapi::aac::AacContext;
use crate::recorders::audio::sources::wasapi::aac::AAC_FRAME_SIZE;
...
use crate::recorders::frame::create_audio_frames;
use crate::recorders::frame::create_av_frame;
...
use crate::recorders::video::sources::d3d111::traits::create_encoder_d3d11;
use crate::recorders::video::sources::d3d111::traits::D3d11EncoderHwContext;
use crate::recorders::video::sources::enums::VideoCodec;
use crate::recorders::video::sources::enums::VideoSourceType;
...
impl<PRB: PacketRingBuffer> Recorder<PRB> {
    ...
    pub fn start_recording(
        ...
        self.recorder
            .take()
            .map(|recorder| recorder.start_capturing(stop_capturing_callback))
    }
}
...
pub fn create_video_recorder<PRB: PacketRingBuffer + 'static>(
    ...
    let codec = match video_codec {
        ...
        VideoCodec::Qsv => {
            return Err(ffmpeg_next::Error::EncoderNotFound.into());
        }
    };
    ...
    let idk = match video_source_type {
        VideoSourceType::D3d11 { monitor_id } => match video_codec {
            VideoCodec::Amf => {
                let d3d11_vs = VideoSourceD3d11::new(*monitor_id, D3d11vaAdapter)?;
                let (hw_device_ctx, hw_frame_ctx) = d3d11_vs
                    .encoder_hw_ctx
                    .setup_hw_and_frame_ctx(&d3d11_vs.device, width as i32, height as i32)?;
                ...
                let av_frame =
                    create_av_frame(AV_PIX_FMT_D3D11, width as i32, height as i32, hw_frame_ctx)?;
                let recorder = VideoRecorder::new(
                    arc_ring_buffer.clone(),
                    d3d11_vs,
                    encoder,
                    MaybeSafeFFIPtrWrapper(av_frame),
                    width,
                    height,
                    fps as f64,
                );
            ...
            VideoCodec::Qsv => {
                ...
                let parameters = Parameters::from(&encoder);
                let av_frame =
                    create_av_frame(AV_PIX_FMT_QSV, width as i32, height as i32, hw_frame_ctx)?;
                let recorder = VideoRecorder::new(
                    arc_ring_buffer.clone(),
                    d3d11_vs,
                    encoder,
...
pub fn create_audio_recorder<PRB: PacketRingBuffer + 'static>(
    ...
    let codec = match audio_code_c {
        AudioCodec::AAC => ffmpeg_next::codec::encoder::find(ffmpeg_next::codec::Id::AAC)
            .ok_or(ffmpeg_next::Error::EncoderNotFound)?, //AudioCodec::Test => { return Err(ffmpeg_next::Error::EncoderNotFound.into()); }
    };
    let ctx = ffmpeg_next::codec::context::Context::new_with_codec(codec);
    ...
    let idk = match audio_source_type {
            ...
            match audio_code_c {
                AudioCodec::AAC => {
                    ...
                    let (frame, silent_frame) =
                        create_audio_frames(sample, AAC_FRAME_SIZE, channel_layout);
                    let encoder = new_audio_encoder_aac(
                        enc,
                        codec,
                        aac_vs.format.nSamplesPerSec as i32,
                        channel_layout,
                        sample,
                    )?;
                    let parameters = Parameters::from(&encoder);
        ...
        AudioSourceType::WasApiProcess {
            ...
            AudioCodec::AAC => {
                let sample = Sample::F32(Type::Planar);
                let aac_vs =
                    AudioSourceWasapi::new_process(AacContext, *process_id, *include_tree)?;
                let channel_layout = match aac_vs.format.nChannels {
                    1 => ChannelLayout::MONO,
                    2 => ChannelLayout::STEREO,
                    _ => return Err(Unknown.into()),

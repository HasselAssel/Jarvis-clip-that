...
impl<PRB: PacketRingBuffer + 'static, VS: VideoSource + Send + 'static> TRecorder<PRB>
    for VideoRecorder<PRB, VS>
{
    fn start_capturing(
        mut self: Box<Self>,
            ...
            for i in 0..u32::MAX {
                ...
                *expected_elapsed = frame_duration.saturating_mul(i);
                ...
                let _ =
                    selbst
                        .video_source
                        .get_frame(&selbst.av_frame, selbst.width, selbst.height);
                ...
                *total_frames_counter += 1;
                ...
                send_frame_and_receive_packets(
                    &selbst.ring_buffer,
                    &mut selbst.video_encoder,
                    &frame,
                    1,
                )
                .unwrap_or_else(|err| {
                    panic!(
                        "VideoRecorder: Failed to send_frame_and_receive_packets because: {:?}",
                        err
                    )
                });
            }
        ...
        thread::spawn(move || {
            self.video_source
                .init()
                .unwrap_or_else(|err| panic!("Failed to init VideoRecorder: {:?}", err));
            ...
            if let Some(stop_capturing_callback) = stop_capturing_callback {
                while stop_capturing_callback.load(Ordering::Relaxed) {
                    help(
                        ...
                        &frame_duration,
                        &mut elapsed,
                        &mut expected_elapsed,
                        &mut start_time,
                        &mut total_frames_counter,
                    );
                }
            } else {
                loop {

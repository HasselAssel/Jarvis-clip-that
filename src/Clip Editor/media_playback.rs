...
use crate::audio_playback::frame_to_interleaved_f32;
use crate::audio_playback::LiveSource;
...
use crate::stream_scheduler::AsyncFnType;
use crate::stream_scheduler::DynRateScheduler;
use crate::stream_scheduler::FixedRateScheduler;
...
impl MediaPlayback {
        ...
        for (index, stream) in media.streams.iter() {
            ...
            match stream.parameters.medium() {
                Type::Video => {
                    ...
                    let scheduler: Box<dyn StreamFrameScheduler<_>> =
                        if params.framerate.num == 0 && params.framerate.den == 1 {
                            Box::new(DynRateScheduler::new(
                            ...
                            Box::new(FixedRateScheduler::new(
                                rate,
                ...
                Type::Audio => {
                    ...
                    let scheduler = Box::new(FixedRateScheduler::new(
                        rate,
                        max_buffered_seconds,
                        call_back,
                        request_new_channel,
    ...
    pub async fn dummy_callback_insert(
        &mut self,
        ctx: eframe::egui::Context,
        video_sender: &std::sync::mpsc::Sender<WorkerMessage>,
    ) -> (Arc<AtomicF32>, HashMap<usize, Arc<AtomicF32>>) {
        if let Some((i, stream)) = self.video_streams.iter_mut().find(|(_, _)| true) {
            let time_base = self.media.streams.get(i).and_then(|stream_info| unsafe {
                ...
                (rational.num != 0 || rational.den != 1)
                    .then(|| rational.den as f32 / rational.num as f32)
            });
    ...
    pub fn get_handles(&self) -> HashMap<usize, StreamHandle> {
        ...
        self.video_streams.iter().for_each(|(i, stream)| {
            stream_handles.insert(*i, stream.get_stream_handle());
        });
        self.audio_streams.iter().for_each(|(i, stream)| {
            stream_handles.insert(*i, stream.get_stream_handle());
        });
    ...
    pub async fn start(mut self, (seeking_seconds_f32, cond): (Arc<AtomicU32>, Arc<AtomicBool>)) {
        self.video_streams.iter_mut().for_each(|(_, stream)| {
            stream.start();
        });
        self.audio_streams.iter_mut().for_each(|(_, stream)| {
            stream.start();
        });
        ...
        loop {
            ...
            self.media
                .ictx
                .seek(index as i64, std::ops::RangeFull)
                .unwrap();

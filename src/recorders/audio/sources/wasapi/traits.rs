...
pub trait WasapiEncoderCtx {
    fn process_audio<PRB: PacketRingBuffer>(&mut self, ring_buffer: &Arc<Mutex<PRB>>, encoder: &mut Encoder, frame: &mut Audio, silent_frame: &mut Audio, packet_length: u32, data: *mut u8, qpc_pos: u64, start_time: i64, frequency: i64, format: &WAVEFORMATEX, pts_counter: &mut i64, audio_buffer: &mut VecDeque<u8>, capture_client: &IAudioCaptureClient);
}

pub fn new_audio_encoder_aac(mut enc: audio::Audio, codec: Codec, rate: i32, channel_layout: ChannelLayout, sample: Sample) -> Result<Encoder> {
    enc.set_rate(rate);
